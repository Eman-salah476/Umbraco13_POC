(function () {
    angular.module("umbraco.directives").component("umbContentGridView", {
        template: '<div class="grid umb-content-grid ng-scope ng-isolate-scope" ng-if="vm.items"><div class="grid-item umb-content-grid__item umb-outline umb-outline--surrounding ng-scope" ng-repeat="item in vm.items track by $index"><div class="umb-content-grid__content"><ul class="umb-content-grid__details-list"> <li class="umb-content-grid__details-item ng-scope" ng-repeat="column in vm.itemProperties"> <div class="umb-content-grid__details-label ng-binding">{{column}}:</div> <div class="umb-content-grid__details-value ng-binding">{{item[column]}}</div></li></ul></div></div></div>',
    ...
    angular.module('umbraco').controller("TableGridViewController", ['$http', '$scope', 'notificationsService', function ($http, $scope, notificationsService) {
        var vm = this;
        ...
        function scrollTop() {
            var obj = $(".umb-table");
            if (obj) {
                var view = obj.closest('.grid-wrapper');
